...
def main():
    ...
    results.append(("Geoscape/Globe.cpp (draw timing)", edit(
        ...
        "{\n"
        "#ifdef __AMIGA__\n"
        "\t/* AMIGA-PORT: hard cap of one full globe redraw per AMIGA_GLOBE_MIN_MS,\n"
        "\t * whatever asked for it. A redraw costs ~330 ms on this hardware (see\n"
        "\t * PROGRESS.md: drawShadow alone is ~280 ms of double-precision maths per\n"
        "\t * pixel through the ROM IEEE libraries), and the game invalidates the\n"
        "\t * globe every time the sun moves - which at any time acceleration is\n"
        "\t * every other frame, so the geoscape ran at ~5 fps and got worse the\n"
        "\t * faster the clock ran. _redraw is deliberately LEFT SET, so a skipped\n"
        "\t * request is serviced by a later call instead of being lost, and the\n"
        "\t * surface is not cleared, so the previous image stays on screen. */\n"
        "\t{\n"
        "\t\tstatic Uint32 lastDraw_ = 0;\n"
        "\t\tUint32 nowTick_ = SDL_GetTicks();\n"
        "\t\tif (lastDraw_ != 0 && (Uint32)(nowTick_ - lastDraw_) < AMIGA_GLOBE_MIN_MS)\n"
        "\t\t\treturn;\n"
        "\t\tlastDraw_ = nowTick_;\n"
        "\t}\n"
        "\tstatic unsigned long calls_ = 0, lastFlips_ = 0;\n"
        "\tstatic Uint32 lastTicks_ = 0, sCache = 0, sOcean = 0, sLand = 0, sRadar = 0,\n"
        "\t              sShadow = 0, sMark = 0, sDetail = 0, sFlight = 0;\n"
        "\tUint32 t0 = SDL_GetTicks(), t1, t2, t3, t4, t5, t6, t7;\n"
        "\t++calls_;\n"
        "#endif\n"
        "\tif (_redraw)\n"
        "\t{\n"
        "#ifdef __AMIGA__\n"
        "\t\t/* AMIGA-PORT: cachePolygons() re-projects every polygon of the world\n"
        "\t\t * (~400 ms here) and depends only on where the globe is pointing and\n"
        "\t\t * how big it is. invalidate() is also called for reasons that change\n"
        "\t\t * neither - hovering the mouse during base placement, for one - and\n"
        "\t\t * that used to pay the re-projection every time. */\n"
        "\t\tif (!_cacheValid || _cacheLon != _cenLon || _cacheLat != _cenLat || _cacheRadius != _radius)\n"
        "\t\t{\n"
        "\t\t\tcachePolygons();\n"
        "\t\t\t_cacheLon = _cenLon;\n"
        "\t\t\t_cacheLat = _cenLat;\n"
        ...
        "\tdrawFlights();\n"
        "#ifdef __AMIGA__\n"
        "\tsCache += t1 - t0; sOcean += t2 - t1; sLand += t3 - t2; sRadar += t4 - t3;\n"
        "\tsShadow += t5 - t4; sMark += t6 - t5; sDetail += t7 - t6;\n"
        "\tsFlight += SDL_GetTicks() - t7;\n"
        "\tif ((calls_ % 10) == 0)\n"
        "\t{\n"
        "\t\tchar b[200];\n"
        "\t\tUint32 now = SDL_GetTicks();\n"
        "\t\tsnprintf(b, sizeof b, \"globe: 10 draws in %lu ms over %lu frames (draw #%lu)\",\n"
        "\t\t\t(unsigned long)(now - lastTicks_), SDLmini_flips - lastFlips_, calls_);\n"
        "\t\tSDLmini_Log(b);\n"
        "\t\tsnprintf(b, sizeof b, \"globe: ms/draw cache %lu ocean %lu land %lu radar %lu\",\n"
        "\t\t\t(unsigned long)sCache/10, (unsigned long)sOcean/10, (unsigned long)sLand/10, (unsigned long)sRadar/10);\n"
        "\t\tSDLmini_Log(b);\n"
        "\t\tsnprintf(b, sizeof b, \"globe: ms/draw shadow %lu markers %lu detail %lu flights %lu\",\n"
        "\t\t\t(unsigned long)sShadow/10, (unsigned long)sMark/10, (unsigned long)sDetail/10, (unsigned long)sFlight/10);\n"
        "\t\tSDLmini_Log(b);\n"
        "\t\tlastTicks_ = now; lastFlips_ = SDLmini_flips;\n"
        "\t\tsCache = sOcean = sLand = sRadar = sShadow = sMark = sDetail = sFlight = 0;\n"
        "\t}\n"
        "#endif\n"
        "}\n",
        "Globe draw timing")))
