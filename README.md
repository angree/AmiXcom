...
3. Start with the `run` script from a Shell (`cd Work:AmiXcom` then `run`) — it sets
   the 1 MB stack the game needs and starts `openxcom-aga`. Do not run the binary from
   Workbench without a stack of at least 1 MB.
